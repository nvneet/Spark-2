...
public class WithInMemoryData_5 {
...
//		Dataset<Row> result = sparkSession.sql("select loglevel, count(datetime) as num from logging_table "
//				+ "group by loglevel order by num");
		
		Dataset<Row> result = sparkSession.sql("select loglevel, collect_list(datetime) as num from logging_table "
				+ "group by loglevel");
